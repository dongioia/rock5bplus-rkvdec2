...
**Status:** **STAGE 1 PASS (complete).** 1a import byte-exact (`STAGE1a-import-bytexact.md`, 3 geometries) + 1b HW-YUV sample CSC-correct (`STAGE1b-hwyuv-sample.md`, flat-chroma PSNR 82/79 dB @ 720p/1088). Zero-copy via PanVK feasible AND pixel-correct. `scripts/vvtest/zc_import_test.c` (both gates, 3× independently reviewed) + `zc.comp` + `zc-run.sh`. **Next = Step 2** (standalone no-copy on-screen pipeline, own spec). Carry-overs: (a) PanVK import bug `drmFormatModifierPlaneCount=2` workaround (upstream-report); (b) ~~DMA-BUF domain bound~~ **RESOLVED (Step-2 Stage-0 PASS, `STAGE0-step2-1080p-dmabuf.md`)**: 1080p IS zero-copyable — Step-1's system-mem fallback was gst copying the padded hw buffer when downstream lacks GstVideoMeta support (NOT CMA/hardware). Step-2 consumer MUST advertise `GST_VIDEO_META_API_TYPE`; (c) edge chroma reconstruction is Mali HW fixed-function (validated visually in Step 2, not CPU-PSNR).
